...
func interact(conn *net.TCPConn) {
	...
	for {
		...
		case "add":
			...
			fmt.Printf("Oy = ")
			fmt.Scan(&data.PointO.CordY)
			fmt.Printf("x = ")
			fmt.Scan(&data.Point1.CordX)
			fmt.Printf("y = ")
			fmt.Scan(&data.Point1.CordY)
			send_request(encoder, "add", &data)
		case "sq":
			send_request(encoder, "sq", nil)
		default:
			send_request(encoder, "comm", nil)
		}
